...
#print(f"Low = {np.where(np.argmax(labels,axis=1)==0)[0].shape[0]}, Medium = {np.where(np.argmax(labels,axis=1)==1)[0].shape[0]}, High = {np.where(np.argmax(labels,axis=1)==2)[0].shape[0]}")
...
df['Label'] = df['Label'].map({0: 'Absent', 1: 'Low', 2: 'Medium',3:'High'})
